...
contract Nut2Earn is ERC20Detailed, Ownable {
    ...
    address public nutfirepit = 0x8421d1560140ad03449df4A66338a7b26Aa380C5;
    ...
    uint256 private constant gonSwapThreshold = TOTAL_GONS / 10000;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToYieldTop = contractTokenBalance.mul(buyyieldFee.add(sellyieldFee)).div(realTotalFee);
        uint256 amountToFirepit = contractTokenBalance.mul(buyfirepitfee.add(sellfirepitfee)).div(realTotalFee);
        uint256 amountToTreasury = contractTokenBalance.sub(amountToLiquify).sub(amountToYieldTop).sub(amountToFirepit);
        ...
        if(amountToTreasury > 0){
         ...
         if(amountToFirepit > 0){
            _swapTokensForBNB(amountToFirepit, nutfirepit);
        }
    ...
    function setSwapBackSettings_liquifyAll(bool isEnabled) external onlyOwner {
        liquifyAll = isEnabled;
    }

    // Set Wallet & Contract Addresses To Receieve Transaction Tax Fees
    function setFeeReceivers(address setnutliquidityReceiver, address setyieldtoptreasuryReceiver, address setnuttreasuryreservesReceiver, address _nutfirepit) external onlyOwner {
    ...
    function setBuyFees(uint256 setbuyliquidityFee, uint256 setbuyyieldFee, uint256 setbuytreasuryFee, uint256 setbuyfirepitFee) external onlyOwner {
        require(
            setbuyliquidityFee <= MAX_FEE_RATE && // $NUT Liquidity Pool Growth
            setbuyfirepitFee <= MAX_FEE_RATE && // $NUT Liquidity Pool Growth
            setbuyyieldFee <= MAX_FEE_RATE && // Yield Treasury Fee
            setbuytreasuryFee <= MAX_FEE_RATE, // $NUT Funding Marketing & Development
            "wrong"
        );
    ...
    // Set Sell Transactions Tax Fees
     function setSellFees(uint256 setsellliquidityFee, uint256 setsellyieldFee, uint256 setselltreasuryFee, uint256 setsellfirepitFee) external onlyOwner {
        require(
            setsellliquidityFee <= MAX_FEE_RATE && // $NUT Liquidity Pool Growth
            setsellfirepitFee <= MAX_FEE_RATE && // $NUT firepit / burn
            setsellyieldFee <= MAX_FEE_RATE && // yieldtop Treasury Reserves
            setselltreasuryFee <= MAX_FEE_RATE, // Nut2Earn Treasury
            "wrong"
        );
